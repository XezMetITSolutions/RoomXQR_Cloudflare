...
export default function DemoReceptionPanel() {
  ...
  // Veri yükleme fonksiyonu (Demo tenant için)
  const loadData = useCallback(async () => {
    try {
      setIsLoading(true);
      
      // Demo tenant için API çağrıları
      const [requestsData, statsData] = await Promise.all([
        DemoApiService.getGuestRequests(),
        DemoApiService.getStatistics(),
      ]);
      
      // Fallback: Mock data kullan
      const finalRequestsData = requestsData.length > 0 ? requestsData : await ApiService.getGuestRequests();
      const finalStatsData = Object.keys(statsData).length > 0 ? statsData : await ApiService.getStatistics();
  ...
  const handleQuickResponse = async (requestId: string, responseType: string, requestType: string) => {
    ...
    try {
      ...
      if (request) {
        ...
        await DemoApiService.sendNotificationToGuest(request.roomId, response.text, 'response');
  ...
  const handleRoomChange = async (fromRoomId: string, toRoomId: string) => {
    try {
      ...
      await DemoApiService.changeRoom(fromRoomId, toRoomId, roomInfo);
  ...
  const processCheckIn = async (roomId: string, guestData: {
    ...
    try {
      const checkInResult = await DemoApiService.checkInGuest(roomId, guestData);
    ...
    try {
      ...
      if (pendingPayments.length > 0) {
        ...
        for (const payment of pendingPayments) {
          await DemoApiService.updatePaymentStatus(payment.id, 'paid');
        }
      ...
      await DemoApiService.clearRoomRequests(roomId);
      
      // Yeni: Müşteri check-out işlemi (QR kod otomatik sıfırlama ile)
      const checkoutResult = await DemoApiService.checkOutGuest(roomId);
      ...
      if (checkoutResult.success) {
        ...
        await DemoApiService.resetRoomQR(roomId);
