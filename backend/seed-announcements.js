...
async function seed() {
    try {
        ...
        if (!tenant) throw new Error('Tenant grandhotel not found');

        const hotel = await prisma.hotel.findFirst({ where: { tenantId: tenant.id } });
        ...
        const announcements = [
            {
                ...
                message: 'Açık büfe akşam yemeğimiz 19:00 - 21:30 saatleri arasında ana restoranımızda servis edilecektir.',
                tenantId: tenant.id,
                hotelId: hotel.id,
                metadata: {
            ...
            {
                type: 'SYSTEM',
                title: 'Spa ve Masaj Hizmetlerinde %20 İndirim!',
            ...
            {
                type: 'SYSTEM',
                title: 'Hoş Geldiniz İkramı: Çay ve Kahve Keyfi',
